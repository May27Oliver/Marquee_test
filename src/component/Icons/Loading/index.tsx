...
const IOS_OS_STRING = 'iOS';
const md = new MobileDetect(navigator.userAgent);
const isIos = md.os() === IOS_OS_STRING;
...
const IOS: React.FC<LoadingProps> = ({ radius = 15 }) => {
  const size = radius * 2;
  return (
    <div className={cx('loader-ios')}>
      <svg
...
const Android: React.FC<LoadingProps> = ({
  color = '#fff',
  strokeWidth = 6,
  radius = 30,
}) => {
  ...
  const size = center * 2;
  return (
    <svg className={cx('circular')} width={size} height={size}>
      <circle
        className={cx('path')}
        cx={center}
        cy={center}
...
export const Loading: React.FC<LoadingProps> = (props) => {
  return isIos ? <IOS {...props} /> : <Android {...props} />;
};
